...
<svelte:head>
  <title>Devenir Talent InterAct - INTERRA</title>
  <meta name="description" content="Devenez Talent InterAct et partagez vos compétences avec notre communauté." />
</svelte:head>
    ...
    <main class="flex-1 md:mt-52">
      <div class="mt-10 w-12/12">
        <h1 class="font-black text-4xl text-vert-interra mb-10 bg-white text-center">
          {data?.agirAvecNou?.titreMembre || 'Devenir Talent InterAct'}
        </h1>

        <article class="mt-10 border p-5 rounded border-orange-interra m-auto max-w-2xl">
          <form name="talent" method="post" data-netlify="true" data-netlify-honeypot="bot-field" class="flex flex-col gap-5">
