...
const formationInterculturellePath = join(__dirname, '..', 'src', 'lib', 'content', 'formation-interculturelle.ts');
...
try {
  ...
  if (data.records.length === 0) {
    console.error('❌ Aucun enregistrement trouvé dans Formation Interculturelle');
    process.exit(1);
  }
  ...
  const imageListeProjets = extractImage(fields.image_liste_projets);

  // Lire le fichier formation-interculturelle.ts existant
  let contentFile = readFileSync(formationInterculturellePath, 'utf8');

  // Mettre à jour l'image
  if (imageListeProjets.url) {
    const imageRegex = /imageListeProjets:\s*\{[\s\S]*?\}/;
    contentFile = contentFile.replace(
      imageRegex,
      `imageListeProjets: {\n      url: "${imageListeProjets.url}",\n      alt: "${imageListeProjets.alt.replace(/"/g, '\\"')}"\n    }`
    );
  }

  writeFileSync(formationInterculturellePath, contentFile, 'utf8');
  console.log('✅ Image Formation Interculturelle mise à jour');
} catch (error) {
