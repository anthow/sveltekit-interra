...
try {
  ...
  if (data.records.length === 0) {
    console.error('❌ Aucun enregistrement trouvé dans Nos projets');
    process.exit(1);
  }
  ...
  const imageDeLaPage = extractImage(fields.image_de_la_page);

  // Lire le fichier nos-projets.ts existant
  let contentFile = readFileSync(nosProjetsPath, 'utf8');

  // Mettre à jour l'image dans l'objet (pas dans le type)
  if (imageDeLaPage.url) {
    // Chercher l'objet nosProjetsContent, pas le type
    const objectRegex = /(imageDeLaPage:\s*\{[\s\S]*?url:\s*')([^']*)('[\s\S]*?alt:\s*')([^']*)('[\s\S]*?\})/;
    contentFile = contentFile.replace(
      objectRegex,
      `$1${imageDeLaPage.url}$3${imageDeLaPage.alt.replace(/'/g, "\\'")}$5`
    );
  }

  writeFileSync(nosProjetsPath, contentFile, 'utf8');
  console.log('✅ Image Nos projets mise à jour');
} catch (error) {
